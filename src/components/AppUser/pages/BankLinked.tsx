...
export default function UserBankLinked() {
  ...
  const [clearState, setClearState] = useState<{ show: boolean; row: AppUser | null }>({ show: false, row: null });
  ...
  const viewDetails = async (id: string) => {
    try { setViewing(await appUserApi.getOne(id)); } catch (error) { console.error(error); }
  };
  ...
  const saveEdit = async () => {
    ...
    updateRow({ ...editing, ...payload } as AppUser);
    setEditing(null);
  };
  ...
  const clearBankDetails = async () => {
    ...
    await appUserApi.update(row.id, {
      bankLinked: false,
      upiId: null,
      bankAccount: null,
      ifsc: null,
  ...
  const toggleLinked = async () => {
    ...
    await appUserApi.update(row.id, { bankLinked: next });
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1400 }}>
      <ConfirmDialog show={confirmState.show} title={confirmState.row?.bankLinked ? 'Unlink Bank Account' : 'Link Bank Account'} message={`Are you sure you want to ${confirmState.row?.bankLinked ? 'unlink' : 'link'} this user bank account?`} onConfirm={toggleLinked} onCancel={() => setConfirmState({ show: false, row: null })} type={confirmState.row?.bankLinked ? 'danger' : 'success'} />
      <ConfirmDialog show={clearState.show} title="Delete Bank Details" message={`Clear all bank and UPI details for ${clearState.row?.name ?? 'this user'}?`} onConfirm={clearBankDetails} onCancel={() => setClearState({ show: false, row: null })} type="danger" />
      <ExportModal show={showExport} onClose={() => setShowExport(false)} title="User Bank Details" fileName="user-bank" getData={() => filtered.map(row => ({ Name: row.name, Code: row.userCode, Phone: row.phone, BankLinked: row.bankLinked ? 'Yes' : 'No', UPI: row.upiId ?? '', AccountHolder: row.accountHolderName ?? '', BankName: row.bankName ?? '', BankAccount: row.bankAccount ?? '', IFSC: row.ifsc ?? '' }))} />
      ...
      <div style={{ background: 'linear-gradient(135deg,#0284C7,#0369A1)', borderRadius: 18, padding: '22px 28px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.75)', marginTop: 4 }}>Manage customer bank account and UPI details</div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: C.bg, borderBottom: `1px solid ${C.border}` }}>
                {['User', 'Code', 'Phone', 'Bank Status', 'UPI / Account', 'Actions'].map(head => <th key={head} style={{ padding: '14px 16px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: C.muted, textTransform: 'uppercase' }}>{head}</th>)}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={row.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                  ...
                  <td style={{ padding: '14px 16px', fontSize: 12, color: C.muted, fontFamily: 'monospace' }}>{row.userCode}</td>
      ...
      {viewing && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, zIndex: 1000 }} onClick={() => setViewing(null)}>
          <div style={{ background: C.card, borderRadius: 16, width: 520, maxWidth: '95vw', padding: 22 }} onClick={e => e.stopPropagation()}>
            {[['Name', viewing.name], ['Code', viewing.userCode], ['Phone', viewing.phone], ['Bank Linked', viewing.bankLinked ? 'Yes' : 'No'], ['UPI ID', viewing.upiId ?? '—'], ['Bank Name', viewing.bankName ?? '—'], ['Account Holder', viewing.accountHolderName ?? '—'], ['Bank Account', viewing.bankAccount ?? '—'], ['IFSC', viewing.ifsc ?? '—'], ['Wallet Balance', `₹${(viewing.walletBalance ?? 0).toLocaleString('en-IN')}`]].map(([key, value]) => (
            ...
            {viewing.upiQrCodeImage ? <a href={viewing.upiQrCodeImage} target="_blank" rel="noreferrer"><img src={viewing.upiQrCodeImage} alt="Customer UPI QR code" style={{ width: 180, height: 180, objectFit: 'contain', display: 'block', margin: '14px auto 0', borderRadius: 10, background: '#fff', border: `1px solid ${C.border}` }} /></a> : null}
          </div>
        </div>
      )}
